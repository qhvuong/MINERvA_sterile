...
namespace {
  TH1D* ConvertAbsHistToFractionUsingParentCV(const PlotUtils::MnvH1D* parent, TH1D* hAbs)
  {
    if (!parent || !hAbs) return hAbs;

    for (int b = 0; b <= hAbs->GetNbinsX() + 1; ++b)
    {
      const double cv = parent->GetBinContent(b);
      const double frac = (cv != 0.0) ? hAbs->GetBinContent(b) / cv : 0.0;
      hAbs->SetBinContent(b, frac);
      hAbs->SetBinError(b, 0.0);
    }
    return hAbs;
  }
}
...
bool MnvPlotter::DrawErrorSummaryDerived(
        MnvH1D* h,
        const std::string& legPos  /* = "TR"    */,
        const bool   includeStat     /* = true    */,
        const bool   solidLinesOnly  /* = true    */,
        ...
        )
{
    if ( ! h )
    {
        Error("DrawErrorSummaryDerived", "You passed me a NULL MnvH1D.  Nothing to do.");
        return false;
    }
    ...
    if (h->HasVertErrorBand(*it_name)) {
    if (asfrac) {
        hErr = dynamic_cast<TH1D*>(
        h->GetVertErrorBand(*it_name)->GetErrorBand(false, covAreaNormalize).Clone(
            Form("tmp_vertErrorAbs_%s", (*it_name).c_str()))
        );
        hErr = ConvertAbsHistToFractionUsingParentCV(h, hErr);
    }
    else {
        hErr = dynamic_cast<TH1D*>(
        h->GetVertErrorBand(*it_name)->GetErrorBand(false, covAreaNormalize).Clone(
            Form("tmp_vertError_%s", (*it_name).c_str()))
        );
    }
    }
    else if (h->HasLatErrorBand(*it_name)) {
    if (asfrac) {
        hErr = dynamic_cast<TH1D*>(
        h->GetLatErrorBand(*it_name)->GetErrorBand(false, covAreaNormalize).Clone(
            Form("tmp_latErrorAbs_%s", (*it_name).c_str()))
        );
        hErr = ConvertAbsHistToFractionUsingParentCV(h, hErr);
    }
    else {
        hErr = dynamic_cast<TH1D*>(
        h->GetLatErrorBand(*it_name)->GetErrorBand(false, covAreaNormalize).Clone(
            Form("tmp_latError_%s", (*it_name).c_str()))
        );
    }
    }
    else if (h->HasUncorrError(*it_name)) {
    hErr = dynamic_cast<TH1D*>(
        h->GetUncorrErrorAsHist(*it_name, asfrac).Clone(
        Form("tmp_uncorr_error_%s", (*it_name).c_str()))
    );
    }
    else {
    throw std::runtime_error(
        Form("MnvPlotter::DrawErrorSummaryDerived(): Couldn't determine error band type for error name '%s'",
            (*it_name).c_str())
    );
    }
      //is this histogram part of a group?
