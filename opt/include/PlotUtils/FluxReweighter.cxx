...
namespace PlotUtils
{
      ...
      int nUniverses/*= 200*/)
  {
    ...
    //   nUniverses = 100;
    // }
    // P8 LE has 1000 universes
    if (LeOrMe(playlist)==1&&nUniverses>1000) {
      nUniverses = 1000;
    }
...
//============================================================================
  ...
  {
    // const char* plotutils=gSystem->Getenv("PLOTUTILSROOT");
    // const char* plotutils = "/pnfs/minerva/persistent/users/qvuong/custom_plotutils";
    const char* base = gSystem->Getenv("CCNUEROOT");
    if (!base || !strlen(base)) {
      std::cout << "$CCNUEROOT is not set. Can't find flux histograms" << std::endl;
      std::exit(1);
    }

    std::string plotutils_str = std::string(base) + "/custom_plotutils";
    const char* plotutils = plotutils_str.c_str();
                                             ...
                                             unsigned int universe )
  {
    ...
    const char* sourceBand = "Flux_BeamFocus";
    // Print only once per process
    static bool printedSetup = false;
    if (!printedSetup)
    {
        std::cout << "\n========== FLUX DIAGNOSTIC ==========\n";
        std::cout << "Event-level output band name : "
                  << m_fluxErrorName << "\n";
        std::cout << "Source flux uncertainty band : "
                  << sourceBand << "\n";

        std::cout << "Available vertical bands:\n";
        for (const auto& name : newFlux->GetVertErrorBandNames())
        {
            std::cout << "  " << name
                      << "  N="
                      << newFlux->GetVertErrorBand(name)->GetNHists()
                      << "\n";
        }

        std::cout << "Requested flux universes      : "
                  << m_nFluxUniverses << "\n";
        std::cout << "=====================================\n\n";
    ...
    double univFluxVal =
        newFlux->GetVertErrorBand(sourceBand)
               ->GetHist(universe)
               ->Interpolate(Enu);
    ...
    if(Enu>75.){//Condition to avoid the large slope at large Enu which causes crazy weights. Slope in that region is 10^-22 or so
      ...
      num = newFlux->GetVertErrorBand(sourceBand)->GetHist(universe)->GetBinContent(newFlux->FindBin(Enu));
    ...
    double cv = newFlux->Interpolate(Enu);

    double ppfxVal =
        newFlux->GetVertErrorBand("ppfx1_Total")
              ->GetHist(universe)
              ->Interpolate(Enu);

    double beamVal =
        newFlux->GetVertErrorBand("Flux_BeamFocus")
              ->GetHist(universe)
              ->Interpolate(Enu);

    double totalVal =
        newFlux->GetVertErrorBand("Flux")
              ->GetHist(universe)
              ->Interpolate(Enu);

    double ppfxWeight =
        (cv != 0.0) ? ppfxVal / cv : 1.0;

    double beamWeight =
        (cv != 0.0) ? beamVal / cv : 1.0;

    double totalWeight =
        (cv != 0.0) ? totalVal / cv : 1.0;

    // This is the weight this function will actually return
    double usedWeight = num / dem;
