...
DO_MNV_TUNE_DIAGNOSTICS = True
MNV_TUNE_DIAG_BINS = None  # or range(1, 13) for EN4 12 bins

def PrintBandUniverseSummary(h, bandname, label="", bins=None):
    """
    Print CV, band CV, and universe spread for one band.
    Works for 1-, 2-, 3-, 100-universe bands.
    """
    if h is None:
        return

    if not h.HasVertErrorBand(bandname):
        print(f"[missing] {label} {h.GetName()} has no band {bandname}")
        return
    ...
    print("\n" + "=" * 120)
    print(f"Band universe summary: {label}  hist={h.GetName()}  band={bandname}  nUniverses={n}")
    print(f"  main integral   = {h.Integral():.8g}")
    print(f"  bandCV integral = {band.Integral():.8g}")
    print(f"  bandCV/main     = {band.Integral()/h.Integral() if h.Integral() else 0.0:.8g}")
    ...
    for b in bins:
        ...
        if cv == 0 and bcv == 0 and all(v == 0 for v in vals):
            continue
        ...
        print(
            "bin {:2d} x=[{:.3f},{:.3f}] "
            "CV={:.6g} bandCV/CV={:.6g} "
            "mean/CV={:.6g} min/CV={:.6g} max/CV={:.6g}".format(
                b,
                h.GetXaxis().GetBinLowEdge(b),
                h.GetXaxis().GetBinUpEdge(b),
                cv,
                bcv / cv if cv else 0.0,
                mean / cv if cv else 0.0,
                vmin / cv if cv else 0.0,
                vmax / cv if cv else 0.0,
            )
        )

        if n <= 5:
            ratios = [(v / cv if cv else 0.0) for v in vals]
            print("    universe/CV:", ["{:.6g}".format(r) for r in ratios])


def PrintMinervaTuneGroupSummary(h, label="", bins=None):
    """
    Print all MINERvA tune/model bands for one histogram.
    """
    print("\n" + "#" * 120)
    print(f"MINERvA tune/model group summary: {label}  hist={h.GetName()}")
    print("#" * 120)

    for group, bands in MINERVA_TUNE_GROUPS.items():
        print("\n" + "-" * 120)
        print("Group:", group)

        for bandname in bands:
            PrintBandUniverseSummary(h, bandname, label=label, bins=bins)


def PrintBandFractionalErrorSummary(h, bandname, label="", bins=None):
    """
    Print the actual fractional uncertainty from PlotUtils for one band,
    using the parent/main CV as the denominator.
    """
    if h is None or not h.HasVertErrorBand(bandname):
        return

    band = h.GetVertErrorBand(bandname)
    err = band.GetErrorBand(True, False)  # as fractional? depends on PlotUtils convention

    if bins is None:
        bins = range(1, h.GetNbinsX() + 1)

    print("\n" + "-" * 100)
    print(f"Band fractional error summary: {label}  hist={h.GetName()}  band={bandname}")

    for b in bins:
        cv = h.GetBinContent(b)
        abs_err = err.GetBinContent(b)

        # If GetErrorBand(True, False) already returns fractional, this will be obvious.
        # To avoid ambiguity, also compute abs/CV if abs_err looks absolute.
        frac_abs_over_cv = abs_err / cv if cv else 0.0

        print(
            "bin {:2d} x=[{:.3f},{:.3f}] CV={:.6g} "
            "GetErrorBand={:.6g}  GetErrorBand/CV={:.6g}".format(
                b,
                h.GetXaxis().GetBinLowEdge(b),
                h.GetXaxis().GetBinUpEdge(b),
                cv,
...
def PrintMinervaTuneFractionalErrors(h, label="", bins=None):
    print("\n" + "#" * 120)
    print(f"MINERvA tune/model fractional error summary: {label}  hist={h.GetName()}")
    print("#" * 120)

    for group, bands in MINERVA_TUNE_GROUPS.items():
        print("\nGroup:", group)
        for bandname in bands:
            PrintBandFractionalErrorSummary(h, bandname, label=label, bins=bins)
...
if __name__ == "__main__":
    ...
    if DO_MNV_TUNE_DIAGNOSTICS:
        print("\n\n" + "@" * 120)
        print("RAW MC MINERvA TUNE DIAGNOSTICS BEFORE BACKGROUND FIT")
        print("@" * 120)

        # Total MC histograms used by the fit
        PrintMinervaTuneGroupSummary(
            mcsignal_histholders[0].GetHist(),
            label="RawMC_Total_SignalRegion_FitHist",
            bins=MNV_TUNE_DIAG_BINS
        )

        # PrintMinervaTuneGroupSummary(
        #     mcsideband_histholders[0].GetHist(),
        #     label="RawMC_Total_dEdXSideband_FitHist",
        #     bins=MNV_TUNE_DIAG_BINS
        # )

        # Category-level histograms most relevant to the 2-component fit
        for cate in ["Total", "NuEElastic", "NCPi0", "CCNuEQE", "CCNuEDelta", "CCNuEDIS"]:
            if cate in mcsignal_histholders[0].hists and mcsignal_histholders[0].hists[cate]:
                PrintMinervaTuneGroupSummary(
                    mcsignal_histholders[0].hists[cate],
                    label=f"RawMC_SignalRegion_{cate}",
                    bins=MNV_TUNE_DIAG_BINS
                )

            # if cate in mcsideband_histholders[0].hists and mcsideband_histholders[0].hists[cate]:
            #     PrintMinervaTuneGroupSummary(
            #         mcsideband_histholders[0].hists[cate],
            #         label=f"RawMC_dEdXSideband_{cate}",
            #         bins=MNV_TUNE_DIAG_BINS
            #     )
    ...
    if DO_MNV_TUNE_DIAGNOSTICS:
        print("\n\n" + "@" * 120)
        print("SCALE-FACTOR MINERvA TUNE DIAGNOSTICS AFTER BACKGROUND FIT")
        print("@" * 120)

        for factor, h in scale_hists.items():
            PrintMinervaTuneGroupSummary(
                h,
                label=f"ScaleFactor_{factor}",
                bins=MNV_TUNE_DIAG_BINS
            )

        for factor, h in scale_hists.items():
            PrintMinervaTuneFractionalErrors(
                h,
                label=f"ScaleFactor_{factor}",
                bins=MNV_TUNE_DIAG_BINS
            )
    ...
    for hist in HISTOGRAMS_TO_UNFOLD:
        ...
        # if DO_MNV_TUNE_DIAGNOSTICS:
        #     print("\n\n" + "@" * 120)
        #     print("BACKGROUND-SUBTRACTED MINERvA TUNE DIAGNOSTICS")
        #     print("@" * 120)

        #     PrintMinervaTuneGroupSummary(
        #         subbedData,
        #         label=f"BkgSubData_{data_hist.plot_name}_{region}",
        #         bins=MNV_TUNE_DIAG_BINS
        #     )

        #     PrintMinervaTuneGroupSummary(
        #         subbedMC,
        #         label=f"PredSignalMC_{data_hist.plot_name}_{region}",
        #         bins=MNV_TUNE_DIAG_BINS
        #     )
