...
def plotRecoKin(mc, chainwrapper, outfile):
    ...
    eventClassifier = EventClassifier(classifiers=["Reco","Truth"] if mc else ["Reco"], use_kin_cuts=False, use_sideband = AnalysisConfig.sidebands)
...
def plotTruthKin(chainwrapper,outfile):
    ...
    eventClassifier = EventClassifier(classifiers=["Truth"],use_kin_cuts=False, use_sideband=[])
...
def CopyMetaTreeToOutPutFile(outfile):
    ...
    # --- Print POT ---
    total_pot = 0
    for entry in copiedtree:
        total_pot += entry.POT_Used
    print("POT in Meta tree:", total_pot)
    # ------------------
    del metatree
    copiedtree.Write()
