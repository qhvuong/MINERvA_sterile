...
def plotRecoKin(mc, chainwrapper, outfile):
    ...
    for counter in range(nEvents):
        ...
        for universe in chain.from_iterable(iter(universes.values())):
            ...
            selected = eventClassifier.side_band is not None or eventClassifier.is_true_signal
            weight = universe.GetWeight()

            if selected and mc and debug_piece_prints < 8:
                universe.DebugStandardWeightPieces(
                    label="counter={} short={}".format(counter, universe.ShortName())
                )
                debug_piece_prints += 1
