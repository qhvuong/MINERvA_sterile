...
void FluxCalculatorLoop::EventLoop(TChain * chain, const TEventList * evtList, PlotUtils::MnvH1D * histogram, std::string branchName, double additionalWeight, bool cvWeighted)
{
  ...
  const bool isIntScaled =
    ((*it_band) == "Flux_BeamFocus" || (*it_band) == "ppfx1_Total");
...
//       }
        for (std::vector<std::string>::const_iterator it_band = vertErrorBandNames.begin();
          ...
          // For cvWeighted histograms, the CV histogram is already filled with
          // mc_cvweight_total, which includes the flux CV contribution.
          //
          // The mc_wgt_* universe arrays appear to be absolute flux weights.
          // Therefore, when filling a universe on top of mc_cvweight_total,
          // use the relative variation:
          //
          //   universe / flux_CV
          //
          // For unweighted histograms, keep the original absolute universe weights.
          double * weightsToFill = universeWeights[*it_band];
          double * relativeWeights = nullptr;

          double cvContr = 1.0;
          if (cvWeightContributions.find(*it_band) != cvWeightContributions.end()) {
            cvContr = cvWeightContributions[*it_band];
          }

          // Protect against bad CV contributors.
          if (cvContr != cvContr || std::abs(cvContr) < 1.0e-12) {
            cvContr = 1.0;
          }

          if (cvWeighted) {
            relativeWeights = new double[1000];

            for (unsigned int univ_idx = 0; univ_idx < nCombinedUniv; univ_idx++) {
              relativeWeights[univ_idx] = universeWeights[*it_band][univ_idx] / cvContr;
            }

            weightsToFill = relativeWeights;
          }

          static int debugWeightPrints = 0;
          if (
              cvWeighted &&
              debugWeightPrints < 10 &&
              ((*it_band) == "ppfx1_Total" || (*it_band) == "Flux_BeamFocus")
          ) {
            std::cout << "\nDEBUG relative flux weight"
                      << " band=" << *it_band
                      << " cvWeight=" << cvWeight
                      << " cvContr=" << cvContr
                      << " absU0=" << universeWeights[*it_band][0]
                      << " relU0=" << weightsToFill[0]
                      << std::endl;
            debugWeightPrints++;
          }
          ...
          if ((*it_band) == "Flux_BeamFocus" || (*it_band) == "ppfx1_Total") {
            if (cvWeighted)
              histogram->FillVertErrorBand(*it_band, cv, weightsToFill, cvWeight, 1.0);
            else
              histogram->FillVertErrorBand(*it_band, cv, weightsToFill);
          }

          for (unsigned int univ_idx = 0; univ_idx < nCombinedUniv; univ_idx++) {
            combinedWeights[univ_idx] *= weightsToFill[univ_idx];
          }

          if (relativeWeights != nullptr) {
