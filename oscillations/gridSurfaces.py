...
def submitJob(tupleName, tag):
  ...
  command = (
    ...
    "/usr/bin/time -v py3env/bin/python3 makeSurface.py "
    "--grid "
    "-m ${PROCESS} "
    "-o $CONDOR_DIR_HISTS "
    "%s "
...
if __name__ == '__main__':
  ...
  if exclude_arg is None or str(exclude_arg).strip().lower() in ["", "none"]:
  ...
  outdir_hists = "/pnfs/minerva/scratch/users/%s/%s_npys" % (
      os.environ["USER"],
      processingID,
  )
