...
IsPi0InFinalState = lambda event: 111 in event.mc_FSPartPDG or -111 in event.mc_FSPartPDG
...
# CC νe backgrounds with correct ordering (wrong-sign first)
TRUTH_CATEGORIES["CCNuEWrongSignQE"] = lambda event: IsCC(event) and IsNue(event) and IsQE(event)
...
# TRUTH_CATEGORIES["CCNuE"]          = lambda event: IsCC(event) and IsNuE(event)
TRUTH_CATEGORIES["CCNuE"]          = lambda event: IsCC(event) and IsNueBar(event)

# TRUTH_CATEGORIES["CCNuMuWrongSign"] = lambda event: IsCC(event) and IsNumu(event)
# TRUTH_CATEGORIES["CCNuMu"]         = lambda event: IsCC(event) and IsNuMu(event)

TRUTH_CATEGORIES["CCNuMuWrongSign"] = lambda event: IsCC(event) and IsNumu(event)
TRUTH_CATEGORIES["CCNuMu"]         = lambda event: IsCC(event) and IsNumuBar(event)
