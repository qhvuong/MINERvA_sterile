...
def dilog(x):
    if x < 0.0 or x > 1.0:
        raise ValueError(f"dilog requires 0 <= x <= 1; got {x}")
    if x == 0.0:
        return 0.0
    if x == 1.0:
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Build a higher-order nu-e elastic MnvH1D prediction "
            "from MINERvA fluxes while preserving the Flux universes."
        )
    )
    ...
    parser.add_argument(
        "--output",
        default="nue_elastic_prediction_higherOrderXS_mnv.root",
    )
